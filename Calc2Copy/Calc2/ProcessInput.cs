...
namespace Calc2
{
    class ProcessInput
    {
        ...
        public static List<string> toNumList(string input)
        {
            ...
            string[] numArray = input.Split(new char[] { '+', '-', '*', '/', '%' });
        ...
        {
            // concatenate any "+" or "-" preceded by "*", "/" or "%", to the following integer

            for (int i = 0; i < inputList.Count; i++)
                if (CheckInput.plusOrMinus(inputList[i]) && CheckInput.timesDivOrMod(inputList[i - 1]))
                    {
